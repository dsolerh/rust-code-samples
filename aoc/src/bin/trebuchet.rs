...
fn main() {
    let sum = calculate_trebuchet(DATA);

    println!("the first sum is: {sum}");
}

fn calculate_trebuchet(data: &str) -> i32 {
    data.lines()
        .map(|s| (s, extract_numbers(s)))
        .inspect(|(s, v)| println!("string: {} number: {}", s, v))
        .map(|(_, v)| v)
        .sum()
}
...
fn extract_numbers(s: &str) -> i32 {
    let mut numbers = s
        .chars()
        .fold("".to_string(), |line, ch| {
            if ch.is_ascii_digit() {
                return format!("{line}{ch}");
            } else {
                let line = format!("{line}{ch}");
                let res = NUM.iter().enumerate().find_map(|(idx, &s)| {
                    if line.ends_with(s) {
                        Some(idx)
                    } else {
                        None
                ...
                if let Some(idx) = res {
                    let num = NUM[idx];
                    let rep = NUM_MAP.get(num).unwrap();
                    let line = line.replace(num, &format!("{}{}", rep, num));
                    return line;
                }
                return line;
            }
    ...
    if let Some(&num) = numbers.get(0) {
        ...
        let digits = format!("{}{}", num, last_num);
...
lazy_static! {
    static ref NUM_MAP: HashMap<&'static str, &'static str> = HashMap::from([
        ("one", "1"),
        ("two", "2"),
        ("three", "3"),
        ("four", "4"),
...
mod tests {
    ...
    #[test]
    fn test_calculate_trebuchet() {
        let sum = calculate_trebuchet(TEST_DATA);

        assert_eq!(sum, 142);

        let sum = calculate_trebuchet(TEST_DATA_2);
    ...
    const TEST_DATA_2: &str = r"two1nine
    eightwothree
    abcone2threexyz
    xtwone3four
    4nineeightseven2
    ...
    fn test_extract_numbers() {
        let num = extract_numbers("pqr3stu8vwx");
        assert_eq!(num, 38);

        let num = extract_numbers("eightwothree");
        assert_eq!(num, 83);

        let num = extract_numbers("eighthree");
        assert_eq!(num, 83);
    }
}

const DATA: &str = r"zlmlk1
